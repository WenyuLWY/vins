...
namespace object_detection
{
    ...
    public:
        ...
        ~ObjectDetectionNodelet() {
            ROS_WARN("ObjectDetectionNodelet destructor");
        }
    ...
    private:
        ...
        virtual void onInit()
        {
            ...
            sub_image = private_nh.subscribe(param.IMAGE_TOPIC, 100, &ObjectDetectionNodelet::image_callback, this);
            sub_depth = private_nh.subscribe(param.DEPTH_TOPIC, 100, &ObjectDetectionNodelet::depth_callback, this);

            pub_cloud = private_nh.advertise<sensor_msgs::PointCloud2>("pcd_cloud", 10);

            depth_detection_thread   = std::thread(&ObjectDetectionNodelet::depth_detection, this);
        }
